...
interface CommandPaletteProps {
  ...
  readonly hasFileViewer: boolean;
  readonly fileViewerOpen: boolean;
  readonly onToggleFileViewer: () => void;
}
...
export function CommandPalette({
  ...
  return (
    ...
    >
      ...
      <div
        className="relative w-full max-w-[520px] rounded-xl border border-border/60 bg-card shadow-2xl overflow-hidden"
      ...
      >
        ...
        >
          <div className="flex items-center gap-2 px-3 border-b border-border/40">
            ...
            />
            <kbd className="text-[10px] text-muted-foreground/30 bg-background/50 px-1.5 py-0.5 rounded border border-border/30 font-mono">
              esc
            </kbd>
          </div>
          <Command.List className="max-h-[320px] overflow-y-auto p-1.5">
            <Command.Empty className="py-6 text-center text-[13px] text-muted-foreground/50">
              No results found.
            </Command.Empty>

            <Command.Group
              heading="Categories"
              className="[&_[cmdk-group-heading]]:text-[10px] [&_[cmdk-group-heading]]:text-muted-foreground/40 [&_[cmdk-group-heading]]:px-2 [&_[cmdk-group-heading]]:py-1.5 [&_[cmdk-group-heading]]:uppercase [&_[cmdk-group-heading]]:tracking-wider [&_[cmdk-group-heading]]:font-medium"
            >
              {categories.map((cat) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[10px] text-muted-foreground/30">{cat.tabs.length} tabs</span>
                  </Command.Item>
                );
              })}
            </Command.Group>
            ...
            {activeTab && (
              ...
              >
                {activeTab.status !== "running" && (
                  <Command.Item
                    value="start process"
                    onSelect={() => { onStartProcess(activeTabId); onOpenChange(false); }}
                    className="flex items-center gap-2.5 px-2 py-1.5 rounded-md text-[13px] text-foreground/80 cursor-pointer data-[selected=true]:bg-background/80 data-[selected=true]:text-foreground transition-colors"
                  >
                    <span className="text-success">Start</span>
                    <span className="text-muted-foreground/40">{activeTab.name}</span>
                  </Command.Item>
                )}
                {activeTab.status === "running" && (
                  <>
                    <Command.Item
                      ...
                      onSelect={() => { onRestartProcess(activeTabId); onOpenChange(false); }}
                      className="flex items-center gap-2.5 px-2 py-1.5 rounded-md text-[13px] text-foreground/80 cursor-pointer data-[selected=true]:bg-background/80 data-[selected=true]:text-foreground transition-colors"
                    >
                      <span className="text-warning">Restart</span>
                      <span className="text-muted-foreground/40">{activeTab.name}</span>
                    </Command.Item>
                    <Command.Item
                      value="stop process"
                      onSelect={() => { onStopProcess(activeTabId); onOpenChange(false); }}
                      className="flex items-center gap-2.5 px-2 py-1.5 rounded-md text-[13px] text-foreground/80 cursor-pointer data-[selected=true]:bg-background/80 data-[selected=true]:text-foreground transition-colors"
                    >
            ...
            {hasFileViewer && (
              ...
              >
                <Command.Item
                  value="toggle file viewer"
                  onSelect={() => { onToggleFileViewer(); onOpenChange(false); }}
                  className="flex items-center gap-2.5 px-2 py-1.5 rounded-md text-[13px] text-foreground/80 cursor-pointer data-[selected=true]:bg-background/80 data-[selected=true]:text-foreground transition-colors"
                >
                  <span>{fileViewerOpen ? "Hide" : "Show"} file viewer</span>
                </Command.Item>
              </Command.Group>
            )}
          </Command.List>
